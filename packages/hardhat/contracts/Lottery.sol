...
contract Lottery is Ownable {
	...
	function getRandomNumber() public view returns (uint256 randomNumber) {
        ...
        // To make it backwards compatible, the old block.difficulty opcode which used to give you the current block's Proof of Work difficulty, is now also returning the prevrandao value.
        // Update: Solidity 0.8.18 now supports block.prevrandao. For any earlier versions, you need to use block.difficulty.
	...
	function returnTokens(uint256 amount) external {
		...
		// TODO
		// Attempt to transfer equivalent ETH back to the user
		(bool success, ) = payable(msg.sender).call{
			value: amount / purchaseRatio
		}("");
